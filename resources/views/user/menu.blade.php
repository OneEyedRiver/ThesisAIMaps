...
<!-- Categories -->
  ...
  @if (!request()->has('cat_name')  &&  !request()->has('search') )
...
<div class="w-[90%] grid  gap-3 grid-cols-2 sm:w-[70%]  sm:gap-4 sm:grid-cols-3  md:grid-cols-4 flex items-center sm:m-a   sm:pl-12  "> 
                ...
                @endforeach
                 ...
                 {{-- Show "See More" card if there are more than 4 products --}}
                @if ($categoryProducts->count() > 2  && !request()->has('cat_name') )
                    <a href="{{ route('show.menu', ['cat_name' => $categoryName]) }}" class="block">
                        <div class="w-full max-w-3xs h-[275px] border border-dashed border-gray-500 p-3 rounded-md flex items-center justify-center shadow hover:shadow-xl hover:-translate-y-2 transition duration-300 cursor-pointer">
                            <span class="text-lg font-semibold text-gray-700">See More →</span>
                        </div>
                    </a>
                @endif
            </div>
